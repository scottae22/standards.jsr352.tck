...
@javax.inject.Named("filterSomeProcessor")
public class FilterSomeProcessor implements ItemProcessor {
	
	private final static Logger logger = Logger.getLogger(FilterSomeProcessor.class.getName());
	...
	public ReadRecord processItem(Object record) throws Exception {
		...
		if (isFailnum(processIteration)){
			...
			System.out.println("Filtering iteration "+processIteration);
			processIteration++;
			return null;
		}
		...
		update = update +1;
		processIteration++;
		System.out.println("processing item "+processedRecord.getCount());
